...
Database::~Database() {
    for(std::pair<std::string,StompFrame*> p:reciept_Frame_map)
    {
        StompFrame* frame=p.second;
        delete frame;
        p.second= nullptr;
    }
...
bool Database::canLogOut() {
    while (reciept_Frame_map.size()!=0)
    {
        std::unique_lock<std::mutex> lock(reciept_frame_lock);
        cv.wait(lock);
    }
    return true;
}
